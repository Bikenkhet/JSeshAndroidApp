...
//            // One day we might propose a caret drawing system ?
    ...
    * jsesh.mdc.model.ModelElementVisitor#visitCartouche(jsesh.mdc.model.Cartouche)
    ...
    public void visitHieroglyph(Hieroglyph h) {
        ...
        ModifiersList listQ = h.getModifiers();
        Iterable<Modifier> iterQ = listQ.asIterable();
        for (Modifier m : iterQ) {
            Optional<Color> c = drawingSpecifications.getTagColor(m.getName());
            if (c.isPresent()) g.setColor(c.get());
            /*  ALTERED
